...
int main() {
    ...
    uint32_t print_interval_us = 166'67; // 約60Hz
    uint32_t next_print_time_us = time_us_32() + print_interval_us;
    while (true) {
        ...
        if (snapshot.last_rx_command == ConvertGcInput::Joybus::Command::Status &&
            time_us_32() >= next_print_time_us) {
            next_print_time_us += print_interval_us;
