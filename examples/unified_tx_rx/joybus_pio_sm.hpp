...
class JoybusPioSm {
  public:
    ...
    // テスト用: 手動で1フレーム送信
    bool send_now(const uint8_t *data, std::size_t nbytes);
  ...
  private:
    void finish_receive_from_irq();
    void start_transmit_from_irq(std::size_t nbytes);
    void on_pio_irq();

    struct IrqMux;
    static IrqMux &irq_mux();
    static void __isr pio0_irq0_handler();
    static void __isr pio1_irq0_handler();
