...
JoybusPioSm::JoybusPioSm(const Config &config) : config_(config) {
    ...
    // 一応プルアップ抵抗を有効化
    gpio_pull_up(config_.pin);
    // 初期化中にLowが送出されないよう無効化
...
void JoybusPioSm::finish_receive_from_irq() {
    ...
    // TODO: 0x01のかわりにJoybusプロトコルの定数定義を使う
    if (rx_work_buffer_[received - 1] != 0x01) {
        rx_bad_ = true;
        return;
    }
