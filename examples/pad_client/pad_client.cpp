...
void PadClient::on_console_reset() {
    pending_console_reset_.store(true, std::memory_order_relaxed);
}
...
void PadClient::tick(uint32_t now_us, const ConsoleState &console) {
    ...
    if (pending_console_reset_.load(std::memory_order_relaxed) && state_ != State::Disconnected &&
    ...
    case State::Resetting: {
        ...
        if (got(Joybus::Command::Reset)) {
            pending_console_reset_.store(false, std::memory_order_relaxed);
    ...
    // 定期Status送信
    case State::Ready: {
        if (pending_console_reset_.load(std::memory_order_relaxed)) {
