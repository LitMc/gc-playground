...
class PadConsoleLink {
    // テスト用
  public:
    Test::TestState &test() { return test_; }
    const Test::TestState &test() const { return test_; }

    void __isr publish_sent_origin_request_from_isr() {
    ...
    [[nodiscard]] bool consume_sent_origin(uint32_t &last_origin_epoch) const {
    ...
    void __isr publish_sent_recalibrate_request_from_isr() {
    ...
    [[nodiscard]] bool consume_sent_recalibrate(uint32_t &last_recalibrate_epoch) const {
