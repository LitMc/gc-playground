...
inline constexpr domain::PollMode to_core_poll_mode(Joybus::PollMode joybus_poll_mode) {
    return static_cast<domain::PollMode>(static_cast<uint8_t>(joybus_poll_mode));
}

inline constexpr domain::RumbleMode to_core_rumble_mode(Joybus::RumbleMode joybus_rumble_mode) {
    return static_cast<domain::RumbleMode>(
        Joybus::clamp_rumble_mode(static_cast<uint8_t>(joybus_rumble_mode)));
}

inline constexpr Joybus::PollMode to_joybus_poll_mode(domain::PollMode core_poll_mode) {
    return static_cast<Joybus::PollMode>(
        Joybus::clamp_poll_mode(static_cast<uint8_t>(core_poll_mode)));
}

inline constexpr Joybus::RumbleMode to_joybus_rumble_mode(domain::RumbleMode core_rumble_mode) {
