"""変換ビューア: S, S⁻¹, S⁻¹+, S∘S⁻¹+, P, C, φ をインタラクティブに可視化する HTML を生成する。
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="S, S⁻¹, S⁻¹+, S∘S⁻¹+, P, C, φ をインタラクティブに可視化する HTML を生成する"
    )
...
<div class="tabs">
    ...
    <button id="tabP" onclick="switchTab('P')">P</button>
...
<div id="panelP" class="tab-panel">
    <canvas id="cvP" width="512" height="512"></canvas>
    <div class="info">
        <span class="legend-input">●</span> s
        <span class="legend-c-out">●</span> C(s)
        <span class="legend-phi-out">●</span> φ(C(s))（目標）
        <span class="legend-sinv-out">●</span> P(s)
        <span class="legend-verify-out">●</span> S(P(s))（実際）
...
function initBgP() {
    const img = ctxP.createImageData(SZ, SZ);
    for (let mx = 0; mx < N; mx++)
        for (let my = 0; my < N; my++) {
            const cx = mx - 128, cy = my - 128;
            const in100 = inOctA(cx, cy, 100);
...
function drawBgP() {
    ctxP.putImageData(bgP, 0, 0);
    drawCenterLines(ctxP);
    drawOctOutline(ctxP, OCT100_VERTS, T.oct100Line, 1.5);
    drawOctOutline(ctxP, OCT125_VERTS, T.oct125Line, 1.5);
}

function drawChain(ctx, pts, colors) {
    for (let i = 0; i < pts.length - 1; i++) {
        const [x1, y1] = g2c(pts[i][0], pts[i][1]);
        const [x2, y2] = g2c(pts[i+1][0], pts[i+1][1]);
        ctx.strokeStyle = T.dashLine;
        ctx.lineWidth = 1;
        ctx.setLineDash([3, 3]);
        ctx.beginPath();
    ...
    for (let i = 0; i < pts.length; i++) {
        dot(ctx, pts[i][0], pts[i][1], colors[i], i === 0 ? 4 : 5);
    }
}
...
function updateP() {
    ...
    const [cx, cy] = clampOct(sx, sy);
    // Step 2: φ(C(s))
    const [px, py] = phi(cx, cy);
    // Step 3: S⁻¹+(φ(C(s)))
    const imx = Math.round(px), imy = Math.round(py);
    const pre = (imx >= 0 && imx < N && imy >= 0 && imy < N) ? INV_SF[imx][imy] : null;

    if (pre) {
        const [rx, ry] = pre;
        // Step 4: S(P(s)) — Switch 2 が実際に受け取る値
        const [mx2, my2] = S_DATA[rx][ry];
        const errE = Math.sqrt((mx2 - px) * (mx2 - px) + (my2 - py) * (my2 - py));

        drawChain(ctxP,
            [[sx, sy], [cx, cy], [px, py], [rx, ry], [mx2, my2]],
            [T.inputDot, T.cOutput, T.phiOutput, T.sinvOutput, T.verifyOutput]);

        label(ctxP, sx, sy, `s (${sx},${sy})`, T.inputDot);
        label(ctxP, cx, cy, `C (${cx.toFixed(1)},${cy.toFixed(1)})`, T.cOutput);
        label(ctxP, px, py, `\\u03c6\\u2218C (${px.toFixed(1)},${py.toFixed(1)})`, T.phiOutput);
        label(ctxP, rx, ry, `P(s) (${rx},${ry})`, T.sinvOutput);
        label(ctxP, mx2, my2, `S(P) (${mx2},${my2}) err=${errE.toFixed(1)}`, T.verifyOutput);
    } else {
        dot(ctxP, sx, sy, T.inputDot, 4);
        label(ctxP, sx, sy, `s (${sx},${sy})`, T.inputDot);
    }
}
...
cvP.addEventListener('mouseleave', () => { curP = [-1, -1]; drawBgP(); });
...
const TAB_MAP = {S:'S', sinv:'Sinv', sinvF:'SinvF', verify:'Verify', P:'P', C:'C', phi:'Phi'};
...
mq.addEventListener('change', () => {
    ...
    initBgS(); initBgSinv(); initBgSinvF(); initBgVerify(); initBgP(); initBgC(); initBgPhi();
