...
- Copilot レビュー指摘は **Teams（reviewer + operator）が採否を判断**する。reviewer が技術的妥当性を評価し、operator が反映を調整する。有効な指摘は反映し、無効・誤認識はスキップ可。
- **2回目以降の push 後は `gh pr comment <number> --body "@copilot re-review"` でレビューを依頼する**（初回のみ自動、以降は手動）。
- Copilot が**フィーチャーブランチに対して**提案 PR を作成した場合は **Teams がレビューしてマージまたはクローズ**する（operator 主担当、ユーザー承認不要）。Teams 自身が作成したフィーチャーブランチの PR は必ずユーザーの承認を得てからマージする。
...
機能ベースで再編し、ハンドオフを最小化する4体構成。エージェント間で直接対話しながら品質を高める。
